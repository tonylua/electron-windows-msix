...
const getBinaries = async (windowsKitPath: string) => {
  ...
  if (!(await fs.exists(binaries.makeAppx)))
    log.error(`MakeAppx binary ${MAKE_APPX_EXE} not found in:`, true, {
      windowsKitPath,
    });
  if (!(await fs.exists(binaries.makePri)))
    log.error(`MakePri binary ${MAKE_PRI_EXE} not found in:`, true, {
      windowsKitPath,
    });
  if (!(await fs.exists(binaries.signTool)))
    log.error(`SignTool binary ${SIGN_TOOL} not found in:`, true, {
      windowsKitPath,
    });
  return binaries;
};
...
const ensureFolders = async (options: PackagingOptions) => {
  ...
  if (await fs.exists(outputDir)) {
    log.debug("Output dir already exists. Making sure its empty.", {
      outputDir,
    });
...
export const getManifestVariables = async (
  ...
  const sparseRegex =
    /<uap10:AllowExternalContent>\s*true\s*<\/uap10:AllowExternalContent>/s;
...
export const verifyOptions = async (
  ...
  if (!options.appManifest)
    log.error("Path to application manifest <appManifest> not provided.", true);
  if (!(await fs.exists(options.appManifest)))
    log.error(
      "Path to application manifest <appManifest> does not exist.",
      true,
      { appManifest: options.appManifest }
    );
  if (!options.appDir && !isSparsePackage)
    log.error("Path to application <appDir> not provided.", true);
  if (!(await fs.exists(options.appDir)) && !isSparsePackage)
    log.error("Path to application <appDir> does not exist.", true, {
      appDir: options.appDir,
    });
  if (!options.packageAssets)
    log.error("Path to packages assets <packageAssets> not provided.", true);
  if (!(await fs.exists(options.appManifest)))
    log.error("Path to packages assets <packageAssets> does not exist.", true, {
      packageAssets: options.packageAssets,
    });
  if (!options.cert && !options.signParams)
    log.warn(
      "Path to cert <cert> and/or signParams not provided. Package will not be signed!"
    );
  if (options.cert && options.signParams)
    log.warn(
      "Path to cert <cert> and signParams provided. signParams will take priority!"
    );
  if (options.cert && !(await fs.exists(options.cert)))
    log.error("Path to cert <cert> does not exist.", true, {
      cert: options.cert,
    });
  if (options.cert && !options.cert_pass)
    log.warn("Cert cert password <cert_pass> not provided.");
  if (options.cert) {
    ...
    if (publisher != certPublisher)
      log.error(
        ...
        false,
        { manifest_publisher: publisher, cert_publisher: certPublisher }
      );
  }
};
...
export const locateMSIXTooling = async (
  ...
  if (windowsKitVersion) {
    ...
    if (
      WindowsVersion.IsOlder(windowsKitVersion, MIN_ARM_WIN_KIT_VERSION) &&
      arch === "arm64"
    ) {
      arch = "x64";
    }
    ...
    const windowsKitPathExec = path.join(
      WIN_KIT_BIN_PATH,
      windowsKitVersion,
      arch
    );
    ...
    if (await fs.pathExists(windowsKitPathExec)) {
      ...
      log.debug(
        `WindowsKit version ${windowsKitVersion} exists. Getting binary paths.`,
        binaries
      );
      return binaries;
    } else {
  ...
  if (appManifest) {
    ...
    console.log(appManifest, osMinVersion, arch);
    if (osMinVersion) {
      // Older versions than WinKit 10.0.22621.0 for ARM are missing the makeAppx.exe and we will fall back to x64 in that case.
      if (
        WindowsVersion.IsOlder(osMinVersion, MIN_ARM_WIN_KIT_VERSION) &&
        arch === "arm64"
      ) {
        arch = "x64";
      }
      log.debug(
        "WindowsKitVersion was derived from OSMiversion of the AppxManifest. Checking if it exists....",
        { windowsKitVersion }
      );
      const windowsKitPathExec = path.join(
        WIN_KIT_BIN_PATH,
...
export const makeProgramOptions = async (
  ...
  const { makeAppx, makePri, signTool } = await locateMSIXTooling(
    options,
    manifestVars
  );
  const { outputDir, layoutDir } = await ensureFolders(options);
  const { appName, packageArch, isSparsePackage } = manifestVars;
  const msiPackageName =
    options.packageName ||
    (packageArch ? `${appName}_${packageArch}.msix` : `${appName}.msix`);
  ...
  if (options.signParams && options.signParams.length > 0) {
    ...
    signParams = ["-fd", "sha256", "-f", options.cert];
